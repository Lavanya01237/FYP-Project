...
class ReinforcementLearningAlgorithm {
  ...
  async findRoute(startLat: number, startLon: number, startHour: number, endHour: number): Promise<Route> {
...
class GreedyAlgorithm {
  ...
  // Find the optimal route using greedy algorithm
  async findRoute(startLat: number, startLon: number, startHour: number, endHour: number): Promise<Route> {
    const locations: Location[] = [];
    let currentLat = startLat;
    let currentLon = startLon;
...
app.post('/api/optimize-route', async (req: express.Request, res: express.Response) => {
  const { startLocation, startTime, endTime, algorithm = 'reinforcement' }: OptimizeRouteRequest = req.body;
  ...
  try {
    ...
    // Use the endTime provided by the client
    let optimizedRoute: Route;
    
    if (algorithm === 'greedy') {
      console.log(`Using greedy algorithm from ${startTime} to ${endTime}`);
      ...
      console.log(`Using reinforcement learning algorithm from ${startTime} to ${endTime}`);
